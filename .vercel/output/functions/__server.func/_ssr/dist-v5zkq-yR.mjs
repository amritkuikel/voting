import { t as capitalizeFirstLetter } from "./string-Bx-0C9B2.mjs";
...
import { f as logger, i as createAdapterFactory } from "./factory-CkTUU_9-.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/dist-v5zkq-yR.js
...
var createKyselyAdapter = async (config) => {
	...
	if ("fileControl" in db) {
		const { BunSqliteDialect } = await import("./bun-sqlite-dialect-DzNwOpKv-BMm6igCT.mjs");
	...
	if ("createSession" in db) {
		...
		if (DatabaseSync && db instanceof DatabaseSync) {
			const { NodeSqliteDialect } = await import("./node-sqlite-dialect-DuZHmWgm.mjs");
			dialect = new NodeSqliteDialect({ database: db });
		}
	}
	if ("batch" in db && "exec" in db && "prepare" in db) {
		const { D1SqliteDialect } = await import("./d1-sqlite-dialect-C2B7YsIT-B5BCU9W0.mjs");
